...
import { getBucketByMime, buildContactMediaBucketPath, buildGroupMediaBucketPath } from "../../utils/publicPath";
...
export const verifyMediaMessage = async (
  ...
  try {
    ...
    try {
      // Determina bucket e diretório relativo por contato/grupo
      const bucket = getBucketByMime(media.mimetype);
      const publicRoot = path.resolve(__dirname, "..", "..", "..", "public");
      const isGroup = !!contact?.isGroup;
      const groupJid = isGroup
        ? (contact.remoteJid && contact.remoteJid.includes("@") ? contact.remoteJid : `${contact.number}@g.us`)
        : "";
      const contactUuid = (contact as any)?.uuid as string | undefined;

      const relativeDir = isGroup
        ? buildGroupMediaBucketPath(companyId, groupJid, bucket)
        : buildContactMediaBucketPath(companyId, contactUuid || String(contact.id), bucket);

      const folder = path.resolve(publicRoot, relativeDir);
    ...
    // Define caminho relativo salvo no banco (relativo à raiz company)
    const bucket = getBucketByMime(media.mimetype);
    const isGroup = !!contact?.isGroup;
    const groupJid = isGroup
      ? (contact.remoteJid && contact.remoteJid.includes("@") ? contact.remoteJid : `${contact.number}@g.us`)
    ...
    const relativeDir = isGroup
      ? buildGroupMediaBucketPath(companyId, groupJid, bucket)
      : buildContactMediaBucketPath(companyId, contactUuid || String(contact.id), bucket);
    // Remover prefixo 'company{companyId}/' para salvar no DB relativo à raiz da company
    const dbRelativeDir = relativeDir.replace(new RegExp(`^company${companyId}/`), "");
    ...
    const messageData = {
      ...
      fromMe: msg.key.fromMe,
      read: msg.key.fromMe,
      mediaUrl: path.posix.join(dbRelativeDir, media.filename),
    ...
    // Atualiza dados do contato (avatar/nome) também quando a mensagem é de mídia
    try {
      ...
      logger.warn("[verifyMediaMessage] Falha ao atualizar avatar do contato", err);
    }
...
export const verifyMessage = async (
  ...
  try {
    ...
    logger.warn("[verifyMessage] Falha ao atualizar avatar do contato", err);
  }
...
export const handleMessage = async (
  ...
  // Variáveis necessárias para o processamento de mensagens
  let msgContact: IMe;
  ...
  // Determina se a mensagem é de grupo
  const isGroup = msg.key.remoteJid?.endsWith("@g.us");

  // Carrega dados da conexão WhatsApp
  const whatsapp = await ShowWhatsAppService(wbot.id!, companyId);

  // Bloqueia grupos se a conexão não permitir
  if (!whatsapp.allowGroup && isGroup) return;

  // Se for grupo, busca metadados e cria/atualiza contato de grupo
  let groupMeta: any | undefined;
  if (isGroup) {
    try {
      groupMeta = await wbot.groupMetadata(msg.key.remoteJid!);
      ...
      // silêncio: grupos podem falhar intermitentemente
    }
  }

  // Obtém contato da mensagem (pessoa)
  msgContact = await getContactMessage(msg, wbot);
  const contact = await verifyContact(msgContact, wbot, companyId);

  // Controle de não lidas em cache
  ...
  // Garante ticket existente para este contato/whatsapp
  ...
  // Variáveis auxiliares utilizadas mais adiante
  let ticketTag: TicketTag | undefined;
  let nextTag: Tag | undefined;
  let rollbackTag: Tag | undefined;
  let bodyNextTag = "";
  let bodyRollbackTag = "";
  let currentSchedule: any;
  let mediaSent: Message | undefined;

  // Busca o último ticket (para uso posterior como isFirstMsg)
  ...
  // (duplicado removido: contato e não lidas já processados acima)
  ...
  try {
    if (!msg.key.fromMe) {
      ...
      if (!isNil(whatsapp.collectiveVacationMessage && !isGroup)) {
        ...
        // Lógica específica permanece mais adiante no código
      }
    }
